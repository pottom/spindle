...
func (m Model) wordsRiding(count int) []int {
	...
	if m.words.beats {
		...
		// would go.
		//
		// Where there is a beat to keep, a round of the walk crosses the row on
		// top of that — struck a mark at a time or rolled across as a wave, out
		// and back, and then the row is left alone for a while. Between the
		// rounds, and in a round dealt no walk at all, there is nothing to add
		// and the row is exactly what it is here. See wordsWalkNow.
		if walk := m.wordsWalkNow(count); walk.walking {
			return m.wordsWalking(count, walk)
		}
...
// wordsWalkKind is how a round of the walk crosses the row.
type wordsWalkKind int

const (
	wordsMarching wordsWalkKind = iota // one mark struck at a time, a step per beat
	wordsWaving                        // a crest with shoulders, rolling across
	wordsStanding                      // no walk at all: the row rides its bands
)

const (
	// wordsRoundStill is how often a round is dealt no walk at all.
	//
	// One in five. A stretch that simply rides its bands is part of what the row
	// does rather than a walk that failed to arrive — three gaps in a row that
	// all walk are three gaps anybody can call in advance — but a row oftener
	// still than moving is a row that has stopped keeping time, which is the
	// thing all of this was built to do.
	wordsRoundStill = 5

	// wordsRoundOneWay is how often a round sweeps one way and rests instead of
	// coming back. One in four: out and back is the figure, and the sweep that
	// walks off the end and leaves the row to itself is what keeps the figure
	// from being the only thing that ever happens.
	wordsRoundOneWay = 4

	// wordsRestLeast is the shortest rest on the end of a walking round, in
	// beats, and wordsRestSpread how many lengths there are to deal from — two
	// beats to seven.
	//
	// The rest is what makes the walk arriving an event rather than wallpaper.
	// Under two beats the row is never really let go and the walk reads as
	// stumbling rather than as stopping. Over about seven the bar it sits in can
	// be over before the walk comes back: the shortest bar of marks this screen
	// puts up is soloLeast, twelve seconds, which is twenty-four beats at a
	// hundred and twenty to the minute — and seven of those is already most of
	// the way through what a short row has time for.
	wordsRestLeast  = 2
	wordsRestSpread = 6

	// The same for a round dealt no walk, which is a rest and nothing else: four
	// beats to eleven, so it reads as a stretch of the record left alone rather
	// than as a pause a shade longer than the last one.
	wordsStillLeast  = 4
	wordsStillSpread = 8

	// wordsWaveWide is how far a wave's swell reaches either side of its crest,
	// as a share of the row, and wordsWaveLeast the fewest marks it ever reaches
	// however short the row is.
	//
	// Somewhat over a quarter of the row: a row of seven has a mark up on either
	// side of the crest and a row of twenty has three or four. Narrower and the
	// marks beside the crest barely leave the ground, which is the march with a
	// soft edge on it; wider and most of the row is up at once, and a row that is
	// all up together has nothing travelling along it.
	wordsWaveWide  = 0.28
	wordsWaveLeast = 2

	// wordsRoundsMost is how many rounds are counted through before the walk
	// gives the bar up.
	//
	// A round is never shorter than wordsRestLeast, so this reaches a thousand
	// beats — eight minutes at a hundred and twenty to the minute, against a bar
	// of marks that is at most a solo long, and stamped afresh every half minute
	// on a record with no words at all.
	wordsRoundsMost = 512
)

// wordsMarchSteps is how long a march holds each mark, in beats.
//
// A mark a beat is the walk this row reads best at and is dealt twice as often
// for that. Two a beat is a run down the row and one every two beats is a walk
// that can be watched arriving; at either the strike keeps its own length,
// because how long a strike takes to rise is a property of the eye rather than
// of the tempo. See wordsStruck.
var wordsMarchSteps = [...]float32{0.5, 1, 1, 2}

// wordsWaveCrossings is how long a crest takes to cross the whole row, in
// beats: a bar of four, or two of them.
//
// The row is the stand and the crest is the wave going round it. Four beats is
// two seconds at a hundred and twenty to the minute, which across a dozen marks
// is six of them a second — quick enough that what travels is a swell rather
// than marks being lit in turn, slow enough that the eye can follow the crest
// from one end to the other. Twice that is the same wave through a slower room.
var wordsWaveCrossings = [...]float32{4, 8}

// wordsRound is the figure one round of the walk is dealt: how it crosses the
// row, which end it starts at, how fast it travels, and how long the row is
// left alone at the end of it.
type wordsRound struct {
	kind  wordsWalkKind
	right bool    // starts at the right-hand end rather than the left
	back  bool    // out and back, rather than one way and done
	step  float32 // beats a mark, marching
	cross float32 // beats to cross the row, waving
	rest  float32 // beats of nothing on the end of it
}

// wordsRoundFor deals one, from the bar it belongs to and from which round of
// that bar it is.
//
// Dealt rather than fixed, and dealt the way a figure is (see faceWhoFor), for
// the same two reasons. A walk that goes the same way every time is a walk
// anybody can call before it happens, and a row doing one thing in every gap of
// every record is wallpaper however good the thing is. And a walk that is
// random is one no test can say anything about and one the same record does not
// play twice — this is arithmetic on the moment the bar sounds, so a record
// plays the same way every time and the numbers can be shown to a reader.
//
// Which round it is goes into the deal as well as the bar, so a long solo does
// not walk the same figure over and over: what a gap does is a sequence of them,
// and none of them says what the next one will be.
func wordsRoundFor(starts int64, round int) wordsRound {
	h := uint64(starts)*0x94d049bb133111eb + uint64(round+1)*0x9e3779b97f4a7c15
	h ^= h >> 30
	h *= 0xbf58476d1ce4e5b9
	h ^= h >> 27
	...
	// Every answer off its own part of the hash, so that no two of them are the
	// same question asked twice: a row that only ever went right to left when it
	// waved would be one figure wearing two hats.
	r := wordsRound{
		right: (h>>7)&1 == 0,
		back:  (h>>11)%wordsRoundOneWay != 0,
		step:  wordsMarchSteps[(h>>17)%uint64(len(wordsMarchSteps))],
		cross: wordsWaveCrossings[(h>>23)&1],
		rest:  float32(wordsRestLeast + (h>>29)%wordsRestSpread),
	}

	switch {
	case h%wordsRoundStill == 0:
		r.kind = wordsStanding
		r.rest = float32(wordsStillLeast + (h>>37)%wordsStillSpread)
	case (h>>3)&1 == 0:
		r.kind = wordsWaving
	}
	return r
...
// travel is how long a round spends crossing the row, in beats.
func (r wordsRound) travel(count int) float32 {
	switch r.kind {
	case wordsStanding:
		return 0
	case wordsWaving:
	...
	return float32(r.strikes(count)) * r.step
}

// strikes is how many marks a march strikes before it is done: every mark of
// the row on the way out, and every one of them again on the way back except
// the two on the ends. The far end is where it turns, and a turn is not standing
// there hitting the same mark twice.
func (r wordsRound) strikes(count int) int {
	if !r.back || count < 2 {
		return count
	}
	return 2*count - 2
...
// mark is which mark of the row a march's nth strike falls on, counted from the
// end the round starts at.
func (r wordsRound) mark(count, n int) int {
	if count < 2 {
		return 0
	}
	at := n
	if at > count-1 {
		at = 2*count - 2 - at // coming back over the same marks
	}
	at = min(max(at, 0), count-1)
	if r.right {
		at = count - 1 - at
	}
	return at
...
// walkAt is where a round has got to, u beats into its travel.
func (r wordsRound) walkAt(count int, u float32) wordsWalk {
	if r.kind == wordsWaving {
		// Out and back as one movement rather than as two: the crest turns at
		// the far end the way a wave turns, without ever standing still on the
		// last mark.
		v := u / r.cross
		if v > 1 {
			v = 2 - v
		}
		at := v * float32(count-1)
		if r.right {
			at = float32(count-1) - at
		}
		return wordsWalk{
			walking: true, wave: true, at: at,
			wide: max(wordsWaveWide*float32(count), wordsWaveLeast),
		}
	}

	// Which strike of the round this is, and how far through its own step it
	// has got. The rise is given the same length in time whatever the pace: it
	// is there so the strike is not a discontinuity, and the eye it is there for
	// does not know what the tempo is.
	n := u / r.step
	return wordsWalk{
		walking: true,
		at:      float32(r.mark(count, int(n))),
		hit:     wordsStruck(n-float32(int(n)), min(beatRise/r.step, 0.5)),
	}
}

// wordsWalk is where the beat has got to along the row this frame: which mark a
// march is striking and how hard, or where a wave's crest is and how far its
// shoulders reach either side of it.
type wordsWalk struct {
	walking bool
	wave    bool
	at      float32 // the mark being struck, or where the crest is between two
	hit     float32 // how hard the strike is burning, 0..1
	wide    float32 // how far either side of the crest the swell reaches, in marks
}

// strike is how much of the walk a given mark has, 0..1 — which is what both
// its lift and its light are put up by.
func (w wordsWalk) strike(mark int) float32 {
	if !w.walking {
		return 0
	}
	if !w.wave {
	...
	// The crest and its shoulders, on a cosine rather than on a straight ramp.
	// What travels has to read as a swell, and a swell with a corner on top of
	// it is a tent: the marks either side would rise and fall in step with the
	// crest instead of handing it on.
	d := float32(math.Abs(float64(float32(mark) - w.at)))
	if d >= w.wide {
		return 0
	}
	return 0.5 * (1 + float32(math.Cos(math.Pi*float64(d/w.wide))))
}

// wordsStruck is how hard a strike is burning: how far through its own step it
// is, and how much of that step it spends rising into the beat.
//
// beatPulse's shape with the length of the step handed to it — full on the beat
// and falling away over what is left, so the mark lands rather than sways. At a
// mark a beat and a rise of beatRise the two are the same number; this is that
// one told what a step is worth when the march is not taking one a beat.
func wordsStruck(u, rise float32) float32 {
	u = min(max(u, 0), 1)
	if u < rise {
		return u / rise
	}
	fall := 1 - (u-rise)/(1-rise)
	return fall * fall
}

// wordsWalkNow is the round the bar of marks is in and where that round has got
// to.
//
// Worked out from the bar's own age on the playback clock and from the beat
// grid, with nothing kept between frames: the rounds a bar walks are a sequence
// dealt from the bar, so where it is now is the lengths of the rounds before
// this one added up. That is a walk forward rather than a division, because
// every round is dealt its own length.
//
// Not walking is an answer rather than a failure. It is what the rest between
// two rounds is, and what a round dealt no walk at all is for the whole of its
// length; and it is what a record with no beat, or the key having turned the
// keeping off, gets from one end of the bar to the other. In every one of them
// the row is drawn exactly as it was drawn before any of this existed.
func (m Model) wordsWalkNow(count int) wordsWalk {
	if count <= 0 {
		return wordsWalk{}
	}
	...
	// when the row first goes up. The walk waits at the end it starts from until
	// it is not.
	age := max(time.Duration(m.wordsClock()-m.words.starts)*time.Millisecond, 0)

	// Counted a little past now, by exactly what a strike spends coming up to a
	...
	t, ok := m.beatsAlong(age, lead)
	if !ok {
		return wordsWalk{}
	}

	from := float32(0)
	for r := range wordsRoundsMost {
		round := wordsRoundFor(m.words.starts, r)

		travel := round.travel(count)
		if t < from+travel {
			return round.walkAt(count, t-from)
		}

		from += travel + round.rest
		if t < from {
			return wordsWalk{} // resting, and the row is left to its bands
		}
	}
	return wordsWalk{}
}

// wordsWalking is the row of marks with a round of the walk crossing it: what
// the walk has reached lifted, the rest left to their own part of the sound.
//
// A step sequencer, or a metronome that has a row to walk rather than a
// pendulum; and when the round is a wave, a crowd in a stand. What the walk has
// jumps whatever its own band is doing — a strike over a quiet band that cannot
// be seen is not a strike — and falls away as the walk moves on, so that by the
// time the next mark has it this one is back among the others. What its band is
// doing is still worth something: two marks struck the same way in the same bar
// look mechanical, and the sound is what keeps them apart.
//
...
// beat.
func (m Model) wordsWalking(count int, walk wordsWalk) []int {
	out := make([]int, count)
	for i := range out {
		ride := m.wordsBeatRide(i, count)

		lift := ride * wordsRestRide
		if s := walk.strike(i); s > 0 {
			// Never below what it would have done untouched: at the end of its
			// turn the strike has fallen to nothing, and a mark that dropped
			// under its own neighbours to get there would read as a hole in the
			// row rather than as the walk having moved on.
			lift = max(lift, (wordsStrikeFloor+(1-wordsStrikeFloor)*ride)*s)
		}
...
func (m Model) wordsMarkPaint(word, count, freqs, levels int) wordPaint {
	...
	strike := m.wordsWalkNow(count).strike(word)
	if strike <= 0 {
		return p
	}
	...
	// by a strike of one it is still on the first, and the strike over the part
	...
	//
	// The wave's shoulders come up the palette with it, by however much of the
	// swell they have. What travels along the row is light as much as height,
	// and a crest that moved without taking its colour with it would be a row of
	// marks jumping in front of a still picture.
	p.level += int8(math.Round(float64(int8(levels-1)-p.level) * float64(strike)))
	return p
}
