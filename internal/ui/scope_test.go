...
func TestHelpHeightDoesNotDependOnTheScope(t *testing.T) {
	for _, full := range []bool{false, true} {
		...
		with := lipgloss.Height(m.help.View(m.keys.forPlayer(true, false, false, false, false, 120)))
		without := lipgloss.Height(m.help.View(m.keys.forPlayer(false, false, false, false, false, 120)))
		if with != without {
